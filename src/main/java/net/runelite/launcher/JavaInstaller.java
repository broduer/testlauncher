...
public class JavaInstaller {
    ...
    private static final File SAVE_LOC = new File(System.getenv("APPDATA") + "/" + LauncherProperties.getLauncherName() + "/");
    ...
    public static void init() {
        ...
        try {
            ...
            SplashScreen.stage(0,"", "Finished Downloading Java");
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    private static File downloadFile(String fileURL, String expectedChecksum) throws IOException {
        ...
        if (responseCode == HttpURLConnection.HTTP_OK) {
            ...
            while ((bytesRead = inputStream.read(buffer)) != -1) {
                ...
                SplashScreen.stage((double) progress / 100, progress + "%", "Downloading Java");
            }
            outputStream.close();
            inputStream.close();
            ...
            if (!verifyChecksum(tempFile, expectedChecksum)) {
                SplashScreen.stage(0,"Checksum verification failed.", "Deleting the downloaded file.");
                tempFile.delete();
